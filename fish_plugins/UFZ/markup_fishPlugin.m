function output_ = markup_fishPlugin(fobj, varargin)
% Yolk - FishPlugin
...
% support@tks3.de
    ...
    % ----- EDIT ONLY FROM HERE -------------------------
    thisPlugin.plugin_tag          = 'manualSelection';
    thisPlugin.plugin_name         = 'Manual';
    ...
    thisPlugin.helpURL             = '';
    thisPlugin.plugin_title        = 'Use the CONTEXT MENU to add/edit points and lines!';
    % ----- ...UNTIL HERE -------------------------------
    ...
    function [parameter, displays, debugs] = ParameterDefinitions(fobj)
        ...
        parameter = {  };% 'eyeCutoff',                'Eye Offset',               'numeric',  0.065,     -inf,   inf,    0.005,       false;...
                        %'rel_length',               'Relative Length',          'numeric',  0.065,       0,      2,      0.01,       false;...
                        %'upperCutoff',              'Relative Vertical Cutoff', 'numeric',  0.4,        0,      1,      0.05,       false;...
...
output = [];
    function calculationResults = CalculationFunction(fobj, parameterValues, debugStatus, displayStatus)
        ...
        %% Return Output
        if isempty(fobj.json.(thisPlugin.plugin_tag))
            calculationResults = struct();
        else
            calculationResults = fobj.json.(thisPlugin.plugin_tag);
        end
...
%                 feval(get(hacki2, 'Callback'), hacki2, [], false)
                calculationResults = fobj.(thisPlugin.plugin_tag);
...
%                 interactionRadius = max([1, round(0.15 * averageCapilaryWidth)]);
                ...
                freeSelector(@manual_mode_button_up_fcn, {'free', ha.axes1}, 10);
...
%             cropped_temp = output.step5_cropped_columnNormalized;
            ...
            hp = findobj([ha.axes1, ha.axes_overlay], 'tag', 'free');
            lx = get(hp, 'XData'); if ~iscell(lx), lx = {lx}; end
            ly = get(hp, 'YData'); if ~iscell(ly), ly = {ly}; end
            point = {};
            shape = {};
            for i = 1 : length(hp)
                if isappdata(hp(i), 'Radius')
                    r = getappdata(hp(i), 'Radius');
                    point{end+1} =  struct('name',   ['free_', num2str(i-1)],...
                                           'x',      lx{i}(1),...
                                           'y',      ly{i}(1),...
                                           'radius', r);
                else
                    shape{end+1} = struct('name', ['free_', num2str(i-1)],...
                                           'x',      lx{i},...
                                           'y',      ly{i});
                end
            end
%            calculationResults.mode = 'manual';
            calculationResults = struct('shape', [shape{:}],...
                                        'point', [point{:}]);
...
%         disp(displayStatus)
        ...
        hold(ha.axes1, 'on')
        if isfield(calculationResults, 'shape')
            for i = 1 : length(calculationResults.shape)
                plot(calculationResults.shape(i).x, calculationResults.shape(i).y, 'g', 'parent', ha.axes1, 'tag', 'free');
            end
        end
        
        if isfield(calculationResults, 'point')
            for i = 1 : length(calculationResults.point)
                plotPoints(calculationResults.point(i).x,...
                           calculationResults.point(i).y,...
                           calculationResults.point(i).radius, 'color', [0,1,0], 'parent', ha.axes1);
            end
        end
        hold(ha.axes1, 'off')
return
...
%             end
            ...
            for i = 1 : length(calculationResults.point)
                if ~isempty(strfind(calculationResults.point(i).name, 'free_'))
                    plotPoints(calculationResults.point(i).x,...
                               calculationResults.point(i).y,...
                               calculationResults.point(i).radius, 'parent', ha.axes1, 'color', [0, 1, 0], 'LineWidth', 1, 'tag', 'free', 'visible', onOrOff);
                end
            end
%        end
        end
...
%                     plot(calculationResults.shape(i).x([1:end,1]), calculationResults.shape(i).y([1:end,1]), 'g', 'parent', ha, 'LineWidth', 1, 'tag', 'free');
%                 end
%             end
%         end
col = [1,0.5373,0];
        if isfield(calculationResults, 'shape')
            for i = 1 : length(calculationResults.shape)
                plot(calculationResults.shape(i).x, calculationResults.shape(i).y, 'color', col, 'parent', ha, 'tag', 'free', 'LineWidth', 2);
            end
        end
        ...
        if isfield(calculationResults, 'point')
            for i = 1 : length(calculationResults.point)
                plotPoints(calculationResults.point(i).x,...
                           calculationResults.point(i).y,...
                           calculationResults.point(i).radius, 'color', col, 'parent', ha, 'LineWidth', 2);
            end
        end
...
%         ysi = y(:, ones(1, length(phi))) + r(:, ones(1, length(phi))) .* sin(phi(ones(1, length(r)), :));
        ...
        for i = 1 : length(hpc)
            set(hp(i), 'UserData', hpc(i), 'tag', 'free');
