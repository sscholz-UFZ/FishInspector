...
% support@tks3.de
    ...
    thisPlugin.helpURL             = 'help\fishContour.html';
    % ----- ...UNTIL HERE -------------------------------
    ...
    function [parameter, displays, debugs] = ParameterDefinitions(fobj)
        ...
        %wocw_default = 5*ceil(mean(abs([cp(strcmp(namy, 'lower_in')).y-cp(strcmp(namy, 'lower')).y,...
        %                                cp(strcmp(namy, 'upper')).y-cp(strcmp(namy, 'upper_in')).y]))) ./ averageCapilaryWidth ;
        %wocw_default = round(wocw_default*100)./100;
        wocw_default = 0.11;
        
        % Default for "imclose_disksize"
        %id_default = round(wocw_default*averageCapilaryWidth/2) / averageCapilaryWidth;
        %id_default = round(id_default*100)./100;
        id_default = 0.06;
        % % % %
                                    
        % ===========   TAG                         NAME                        TYPE        DEFAULT         MIN     MAX     INCREMENT   HasAuto
        parameter = {   'auto_thresh_multiplier',   'auto_thresh_multiplier',   'numeric',  0.35,           0,      1,      0.05,       false;...
                        'width_of_capillary_wall',  'width_of_capillary_wall',  'numeric',  wocw_default,   0,      0.5,    0.01,       false;...
                        'imclose_disksize',         'imclose_disksize',         'numeric',  id_default,     0,      1,      0.01,       false;...
                        'binary_contour_smoothing', 'binary_contour_smoothing', 'numeric',  0.21,           0,      Inf,    0.025,      false;...
                        'min_peak_width',           'min_peak_width',           'numeric',  0.01,           0,      1,      0.01,       false};
...
% debug_here = true;
        ...
        averageCapilaryWidth = round(mean(cp(strcmp(namy, 'lower')).y - cp(strcmp(namy, 'upper')).y));
        width_of_capillary_wall = round(parameterValues.width_of_capillary_wall*averageCapilaryWidth);
        ...
        data = imclose(data, strel('disk', round(parameterValues.imclose_disksize*averageCapilaryWidth)));
        data = bwfill(data, 'holes');
...
%                     set(h_, 'xdir', 'normal', 'ydir', 'normal');
%                 case 'ATRA_1_2 (4)_rot3.png'
%                     set(h_, 'xdir', 'reverse', 'ydir', 'normal');
%                 otherwise
%                     set(h_, 'xdir', 'normal', 'ydir', 'reverse');
%             end
        end
        ...
        width = mean(cp(strcmp(namy, 'lower_in')).y - cp(strcmp(namy, 'upper_in')).y);
        b_global_x = b{1}(:,2);
    ...
    function fineContour = detectFineContour(bf_x, bf_y, fobj, parameterValues, debugStatus)
        ...
        [ixi2, ysi2] = contour_smooth(bf_x, bf_y, max([1, round(averageCapilaryWidth*parameterValues.binary_contour_smoothing)]));
%%
...
if 0
    ...
    switch [f,e]
        ...
        % Exclude Points outside capillary
        cp = fobj.capillary.shape; namy = {cp.name};
        upper_max = interp1(cp(strcmp(namy, 'upper_in')).x, cp(strcmp(namy, 'upper_in')).y, global_x);
        lower_max = interp1(cp(strcmp(namy, 'lower_in')).x, cp(strcmp(namy, 'lower_in')).y, global_x);
        
        global_y(upper_max>global_y) = upper_max(upper_max>global_y);
        global_y(lower_max<global_y) = lower_max(lower_max<global_y);
    ...
    function manualModeFunction__(fobj, ha, modeStatus, calculationResults,  parameterValues, debugStatus, ManualSelectionUpdateFCN);
    ...
    function manualModeFunction(fobj, ha, modeStatus, calculationResults,  parameterValues, debugStatus, ManualSelectionUpdateFCN)
        ...
        hacki = [thisPlugin.parameterClassObject.handles.checkbox_plot_binary_shape,...
                 thisPlugin.parameterClassObject.handles.editDisplayOpacity_plot_binary_shape,...
                 thisPlugin.parameterClassObject.handles.labelOpacity_plot_binary_shape,...
                 thisPlugin.parameterClassObject.handles.checkbox_plot_binary_contour];
        while ~strcmp('figure', get(hf, 'Type')), hf = get(hf, 'parent'); end
        ...
        switch modeStatus
            ...
            case {'manual', 'off', 0, false}
                ...
                lineselector2(@manual_mode_button_up_fcn, 'fineContour', interactionRadius);
...
%         disp(displayStatus)
        ...
        if strcmp(calculationResults.mode, 'auto') &&...
           displayStatus.plot_binary_shape
        ...
        % % % %
        % Display Image  
        imh = imshow(im0, 'parent', ha.axes1);
        ...
        if displayStatus.plot_fine_contour
            ...
            % Draw it
            hold(ha.axes1, 'on')
                plot(fc.x, fc.y,  'parent', ha.axes1, 'tag', 'fineContour');
            hold(ha.axes1, 'off')
        end
    ...
    function MainDrawingFunction(fobj, ha, calculationResults)
        fc = calculationResults.shape(strcmp({calculationResults.shape.name}, 'fineContour'));
        %fc = calculationResults.shape(strcmp({calculationResults.shape.name}, 'binaryShape'));
