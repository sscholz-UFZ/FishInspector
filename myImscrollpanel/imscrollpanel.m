...
imageChangedCallback = @() disp([]);
if length(varargin)>2
    imageChangedCallback = varargin{3};
end

% ------------- Support changes of the image object by simple imshow command
try
    a = addlistener(hAx, 'MarkedClean', @AxesChildrenChangedCallback);  % hg2 way
    b = addlistener(hAx, 'Reset', @AxesChildrenChangedCallback2);  % hg2 way
catch Me
    a = handle.listener(handle(hAx, 'ObjectChildAdded', @AxesChildrenChangedCallback)); % hg1 way
end
setappdata(hAx, 'Listeners_Add', a);
setappdata(hAx, 'Listeners_Reset', b);
...
setappdata(hScrollpanel,'API',api);
    ...
    function AxesChildrenChangedCallback2(varargin)
        %imageChangedCallback();
        %disp(['2 - ', num2str(rand)])
    end
    %-------------------------------
...
%         if strcmpi('reverse', get(get(hIm, 'parent'), 'Xdir'))
%             cx = size(get(hIm, 'CData'), 2)-cx;
%         end
%         if strcmpi('reverse', get(get(hIm, 'parent'), 'Ydir'))
%             cy = size(get(hIm, 'CData'), 1)-cy;
%         end
