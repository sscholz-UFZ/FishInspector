...
% ------------- Support changes of the image object by simple imshow command
oldImdata = get(targetImage, 'CData');
targetAxis = ancestor(targetImage, 'axes');
try
    a = addlistener(targetAxis, 'MarkedClean', @AxesChildrenChangedCallback);  % hg2 way
catch Me
    a = handle.listener(handle(targetAxis, 'ObjectChildAdded', @AxesChildrenChangedCallback)); % hg1 way
end
setappdata(targetAxis, 'Listeners_Add', a);
...
if (nargout==1)
    ...
    %----------------------------------------------------
    % Support changes of the image object
    function AxesChildrenChangedCallback(varargin)

        ch = findobj(targetAxis, 'type', 'image');
        if isempty(ch)
            targetImage = [];
        elseif targetImage~=ch(1)
            targetImage = ch(1);
            
            % delete old axes and reinitialize panel
            if ishghandle(hOverviewAx)
                % create listeners and register tool handle
                if isappdata(hOverviewPanel, 'ObjectDestroyedListeners')
                    rmappdata(hOverviewPanel, 'ObjectDestroyedListeners');
                    rmappdata(hOverviewPanel, 'CDataChangedListeners');
                end
                reactToImageChangesInFig(targetImage,hOverviewPanel,@reactDeleteFcn,...
                                         @reactRefreshFcn);
                registerModularToolWithManager(hOverviewPanel,targetImage);
                if ~isequal(get(targetImage, 'CData'), oldImdata)
                    delete(hOverviewAx);
                    drawnow;
                    initializePanel;
                end
 ...
 if strcmpi('reverse', get(get(targetImage, 'parent'), 'Xdir'))
    set(hOverviewIm, 'CData', fliplr(get(targetImage, 'CData')))
%     set(hOverviewAx, 'Xdir', 'reverse');
 else 
    set(hOverviewIm, 'CData', (get(targetImage, 'CData')))
 end
...
%     set(hOverviewAx, 'Xdir', 'reverse');
    ...
    %newpos(1) = size(get(targetImage, 'CData'), 2)-newpos(1)-newpos(3);
